...
atus_string_samp <- sample_n(atus_string, size = 25000, weight = demographics$ID, replace = TRUE)
```
...
dist_matrix[is.infinite(dist_matrix)] <- 100
# hist(dist_matrix)
...
hcl_ch$All.index %>% plot(type = 'l', x = 4:20, main = 'Hierarchical: Silhouette', xlab = 'n clusters')
```

```{r}
# get labels from model
groupings <- cutree(hcl_ward, 8)
...
atus_samp %>% 
  ...
  tally() %>%
  mutate(prop = n / sum(n)) %>% 
  ggplot(aes(x = description, y = prop, fill = description)) +
  geom_col() +
...
labels <- as.character(seq(0, 24, by = 2))
labels <- c(labels[3:13], labels[1:2])
...
```{r}
save_plot(name = "Sequence_plots", height = 8, width = 10)
```
